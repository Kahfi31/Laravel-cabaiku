...
<head>
    ...
    <style>
        ...
        .header-background {
    position: relative;
    background: url('{{ asset("images/download.jpeg") }}') no-repeat center center;
...
<body>
        ...
        <div class="header-container">
            ...
            <nav class="menu-container">
                <ul class="nav">
                    ...
                    <li><a href="{{ url ('/daftarjenis') }}" class="active">Jenis</a></li>
    ...
    <h1 class="header-title">Daftar Artikel Jenis</h1>

<div class="container my-5">
    <div class="row justify-content-center">
        <div class="col-md-10">
            @foreach($articles as $article)
            @if($article->category == 'jenis')
            <div class="article-preview">
                <img src="{{ asset('storage/' . $article->image) }}" alt="{{ $article->title }}">
                <div class="article-content">
                    <h2>
                        <a href="{{ route('jenis.show', $article->id) }}" class="text-decoration-none text-dark">
                            {{ $article->title }}
                        </a>
                    </h2>
                    <p class="text-muted">Dipublikasikan pada {{ $article->created_at->format('d M Y') }}</p>
                    <p>{{ Str::limit($article->content, 150) }}
                        <a href="{{ route('jenis.show', $article->id) }}">Baca selengkapnya</a>
                    </p>
                </div>
            </div>
            @endif
